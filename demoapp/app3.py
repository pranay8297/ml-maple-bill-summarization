...
print("y", bill_number)
mgl_df = pd.read_csv("demoapp/12_bills_with_mgl_more_sections.csv")

def retrieve_mgl(bill_number):
    print("2--", bill_number)
    API_KEY = st.session_state["OPENAI_API_KEY"]
    os.environ['OPENAI_API_KEY'] = API_KEY
    
    mgl_ref = mgl_df.loc[mgl_df['BillNumber']== bill_number, 'combined_MGL_y']
    print("1", len(mgl_ref))
...
def generate_categories(mgl_retriever, text):
    """
    ...
    # mgl_retriever = retrieve_mgl(bill_number)
    
    # LLM
    category_prompt = """According to this list of category {category}.

        classify this bill {context} into a closest relevant category. 
        Use the mass general law sections and chapters provided in the {mgl_context} for more accurate category generation

        Do not output a category outside from the list
    """

    prompt = PromptTemplate(template=category_prompt, input_variables=["context", "category, mgl_context"])

    
    llm = LLMChain(
            llm = ChatOpenAI(openai_api_key=API_KEY, temperature=0, model='gpt-4'), prompt=prompt)
    
    response = llm.predict(context = text, category = category_for_bill, mgl_context = mgl_retriever) # grab from tagging.py
    return response


def generate_response(mgl_retriever, text, category):
    ...
    # retriever = retrieve_mgl(bill_number)   
    ...
    rag_chain = (
            {"context": mgl_retriever, "question": RunnablePassthrough()}
            | prompt
            | llm
            | StrOutputParser()
        )
...
with answer_container:
    ...
    if submit_button:
        with st.spinner("Working hard..."):
            mgl_retriever = retrieve_mgl(bill_number)
            category_response = generate_categories(mgl_retriever, bill_content)
            response= generate_response(mgl_retriever, bill_content, category_response)
